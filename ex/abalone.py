...
if __name__  == "__main__":
    ...
    # Convert categorical variable "Sex" to numerical (One-hot encoding not used for simplicity)
    df["Sex"] = df["Sex"].map({"M": 0, "F": 1, "I": 2})  # Map M=0, F=1, I=2
    
    # Target variable: Age (Rings + 1.5)
    df["Age"] = df["Rings"] + 1.5
    df.drop("Rings", axis=1, inplace=True)

    # Choose feature (e.g., Length) for prediction
    X = df[["Length"]].values
    y = df[["Age"]].values
